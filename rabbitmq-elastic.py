...
JSON_CONFIG_LOCATION = "/usr/local/bin/elasticlogger/myconfig.json"
...
#    ES_BULK_TIMEOUT = 60
    ...
    def __init__(self, amqp_url, es_hosts):
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        try:
            ...
            #inserting
            action = {"_index" : "iotegg", "_type" : "measurements", "_source": data_json}
            self._actions.append(action)
            if (len(self._actions) >= self.ES_BULK_SIZE):
...
def main():
    ...
    global LOG_FILENAME
    ID = '0'
    parser = argparse.ArgumentParser(description="RabbitMQ to ElasticSearch Storage Logging Service")
    ...
    ES_HOST = str(config_data["ElasticSearchHost"])
    ES_PORT = int(config_data["ElasticSearchPort"])
    ES_HOSTS = [{'host':ES_HOST,'port':ES_PORT}] # can add more..
    consumer = ElasticSearchConsumer(AMQP_URL, ES_HOSTS)
    try:
        consumer.run()
